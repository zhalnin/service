...
function printPage($postbody)
{
    ...
    $postbody = preg_replace_callback(
                "|([a-zа-я\d!]{35,})|i",
                '\imei_service\view\utils\split_text',
                $postbody);
    ...
    $postbody = preg_replace_callback($pattern,
                             '\imei_service\view\utils\bold_replace',
                             $postbody);
    // Курсив
    $pattern = '#\[i\](.+)\[\/i\]#isU';
...
//    $pattern = "#\[url[\s]*=[\s]*([\S]+)[\s]*\][\s]*(.*)\[\/url\]#isU";
    ...
    $postbody = preg_replace_callback($pattern,
                                        '\imei_service\view\utils\img_replace',
                                        $postbody);
    // Цвет
    $pattern = '#\[color=[\s]*([a-z]+|\#[0-9a-f]{3}[0-9a-f]{3})[\s]*\](.*)\[\/color\]#isU';
    $postbody = preg_replace_callback($pattern,
                             '\imei_service\view\utils\font_replace',
                            $postbody);
    // Размер
    $pattern = '#\[size=([0-9]+(%|px|em)?)\](.*)\[\/size\]#isU';
    $postbody = preg_replace_callback($pattern,
                                      '\imei_service\view\utils\size_replace',
                                        $postbody);
    // Почта
    $pattern = '#\[mail\](.*)\[\/mail\]#isU';
    $postbody = preg_replace_callback($pattern,
                                        '\imei_service\view\utils\mail_replace',
                                        $postbody);
