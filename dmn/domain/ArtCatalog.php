...
require_once( "dmn/mapper/ArtCatalogIdentityObject.php" );
require_once( "dmn/mapper/ArtCatalogUpDownFactory.php" );
//require_once( "dmn/domain/ArtCatalogPosition.php" );
require_once( "dmn/base/Registry.php" );

class ArtCatalog extends DomainObject {
    private $name;
    private $description;
    private $keywords;
    private $modrewrite;
    ...
    static function findAll() {
        ...
        $catalogIdobj = new \dmn\mapper\ArtCatalogIdentityObject(); // здесь без фабрики создаем экземпляр, чтобы передать имя поля для класса IdentityObect
//        echo "<tt><pre>".print_r($catalogIdobj, true)."</pre></tt>";
    ...
    static function findParent( $id ) {
        $finder = self::getFinder( __CLASS__ );
        $idobj = new \dmn\mapper\ArtCatalogIdentityObject( 'id_parent' );
